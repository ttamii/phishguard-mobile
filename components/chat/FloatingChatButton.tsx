...
const SYSTEM_PROMPT = `Ты — AI-помощник по кибербезопасности в приложении PhishGuard, разработанном для пользователей Казахстана.

ПРАВИЛА:
1. Отвечай на вопросы о фишинге, кибербезопасности, защите данных, работе PhishGuard и кибербезопасности в Казахстане.
2. На вопросы НЕ связанные с кибербезопасностью отвечай: "Я помогаю только с вопросами кибербезопасности. Спросите меня о фишинге или защите данных! 🛡️"
3. ВАЖНО: Отвечай РОВНО 3 короткими предложениями. Не больше. Без списков, без заголовков, без markdown. Только простой текст. На русском. Можно эмодзи.

О PhishGuard: ML модель XGBoost, точность 94.5%, анализ 20 лексических признаков URL, SHAP объяснение.
О Казахстане: рост фишинга 25-30% ежегодно, основные каналы — WhatsApp, Telegram, SMS. Закон «О персональных данных и их защите».`;
...
async function callGemini(messages: { role: string; content: string }[]): Promise<string> {
    ...
    try {
        // Формируем contents — только чередующиеся user/model роли
        ...
        const response = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`,
            {
                ...
                body: JSON.stringify({
                    // Системный промпт — отдельное поле, не в contents
                    systemInstruction: {
