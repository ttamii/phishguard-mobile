...
// ============================================================
const GEMINI_API_KEY = 'AIzaSyBxFqwn2mwv_cjeTSk9wctw90FAXN4HgyM';

const SYSTEM_PROMPT = `Ты — AI-помощник по кибербезопасности в приложении PhishGuard для обнаружения фишинга.

Твои задачи:
1. Отвечать на вопросы о фишинге и кибербезопасности
2. Объяснять как распознать фишинговые атаки
3. Давать советы по защите личных данных
4. Помогать пользователям понять результаты проверки URL

Отвечай кратко (2-4 предложения), на русском языке, дружелюбно.
Используй эмодзи для акцентов.

О приложении PhishGuard:
- Использует ML модели (XGBoost, Random Forest) для анализа URL
- Точность обнаружения: 94.5%
- Анализирует 111 признаков URL
...
function getFallbackResponse(msg: string): string {
    ...
    if (m.includes('работает') || m.includes('анализ') || m.includes('модел')) {
        return '🤖 PhishGuard использует **XGBoost** модель (94.5% точность) для анализа 111 признаков URL: длина, домен, HTTPS, подозрительные слова и др. SHAP объясняет почему URL опасен.';
    }
